class Quest:
    ...
    def interact_eagle(player):
        if "학교 소식 확인" in player.quests and player.quests["학교 소식 확인"].status != "completed":
    ...
    def interact_main_building(player, event_answers):
        if "교내 부조리 수사" in player.quests and player.quests["교내 부조리 수사"].status != "completed":
            print("해결할 임무가 없습니다.")
            return
        
        if player.quests["교내 부조리 수사"].status == "completed":
            print("이미 해결한 임무입니다.")
            return
    ...
    def interact_classroom(player):
        ...
        if corruption_done and food_done:
            ...
            print("부조리 수사를 완료했습니다! 식중독 원인도 찾아주세요.")
