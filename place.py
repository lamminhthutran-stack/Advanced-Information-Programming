class Place:
    def __init__(self, name, buy_items=None, sell_items=None, event_info=None):
        self.name = name
    ...
    def interact(self, player):
        if not self.buy_items:
            print("구매할 수 있는 물선이 없습니다.")
            return
        while True:
            print("무엇을  구매하시겠습니까?")
            items = list(self.buy_items.items())
            for i, (name, (price, hp)) in enumerate(items, 1):
                print(f"{i}. {name} - 가격: {price}원, HP: {hp}만큼 증가한다.")
            ...
            if choice.isdigit() and 1 <= int(choice) <= len(items):
                ...
                if player.money < price:
                    ...
                    print(f"{item_name}을 구매해서 가방에 넣었다. 계좌 잔액 = {player.money}원")
            else:
                print("올바른 번허를 입력하세요.")
    ...
    def interact_sell(self, player):
        if not self.sell_items:
            ...
            return
        sellable = [item for item in player.inventory if item in self.sell_items]
        ...
        while True:
            ...
            if choice.isdigit() and 1 <= int(choice) <= len(unique_items):
                ...
                price, hp = self.sell_items[item_name]
